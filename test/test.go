...
func main() {
	...
	start := time.Now()
	defer func() {
		fmt.Println("\nExecute in: ", time.Since(start))
	}()

	color.Cyan("1. Atoi test")
	testresult("", 0, student.Atoi)
	testresult("-", 0, student.Atoi)
	testresult("--123", 0, student.Atoi)
	testresult("1", 1, student.Atoi)
	testresult("-3", -3, student.Atoi)
	testresult("8292", 8292, student.Atoi)
	testresult("9223372036854775807", 9223372036854775807, student.Atoi)
	testresult("-9223372036854775807", -9223372036854775807, student.Atoi)
	fmt.Println()

	color.Cyan("2. RecursivePower test")
	testresult2(-7, -2, 0, student.RecursivePower)
	testresult2(-8, -7, 0, student.RecursivePower)
	testresult2(4, 8, 65536, student.RecursivePower)
	testresult2(1, 3, 1, student.RecursivePower)
	testresult2(-1, 1, -1, student.RecursivePower)
	testresult2(-6, 5, -7776, student.RecursivePower)
	fmt.Println()

	color.Cyan("3. PrintCombN test")
	...
	color.Cyan("4. PrintNbrBase test")
	testresultstr(919617, "01", "11100000100001000001", student.PrintNbrBase)
	testresultstr(753639, "CHOUMIisDAcat!", "HIDAHI", student.PrintNbrBase)
	testresultstr(-174336, "CHOUMIisDAcat!", "-MssiD", student.PrintNbrBase)
	testresultstr(-661165, "1", "NV", student.PrintNbrBase)
	testresultstr(-861737, "Zone01Zone01", "NV", student.PrintNbrBase)
	testresultstr(125, "0123456789ABCDEF", "7D", student.PrintNbrBase)
	testresultstr(-125, "choumi", "-uoi", student.PrintNbrBase)
	testresultstr(-9223372036854775808, "0123456789", "-9223372036854775808", student.PrintNbrBase)
	fmt.Println()

	color.Cyan("5. doop test")
	testdoop("1", "+", "1", "2")
	testdoop("hello", "+", "1", "0")
	testdoop("1", "p", "1", "0")
	testdoop("1", "/", "0", "No division by 0")
	testdoop("1", "%", "0", "No remainder of division by 0")
	testdoop("9223372036854775807", "+", "1", "0")
	testdoop("-9223372036854775809", "-", "3", "0")
	testdoop("9223372036854775807", "*", "3", "0")
	testdoop("1", "*", "1", "1")
	testdoop("1", "*", "-1", "-1")
	fmt.Println()

	color.Cyan("6. AtoiBase test")
	testatoibase("bcbbbbaab", "abc", 12016, student.AtoiBase)
	testatoibase("0001", "01", 1, student.AtoiBase)
	testatoibase("00", "01", 0, student.AtoiBase)
	testatoibase("saDt!I!sI", "CHOUMIisDAcat!", 11557277891, student.AtoiBase)
	testatoibase("AAho?Ao", "WhoAmI?", 406772, student.AtoiBase)
	testatoibase("thisinputshouldnotmatter", "abca", 0, student.AtoiBase)
	testatoibase("125", "0123456789", 125, student.AtoiBase)
	testatoibase("uoi", "choumi", 125, student.AtoiBase)
	testatoibase("bbbbbab", "-ab", 0, student.AtoiBase)
	fmt.Println()

	color.Cyan("7. SplitWhiteSpaces test")
	...
	color.Cyan("9. ConvertBase test")
	testconvertbase("4506C", "0123456789ABCDEF", "choumi", "hccocimc", student.ConvertBase)
	testconvertbase("babcbaaaaabcb", "abc", "0123456789ABCDEF", "9B611", student.ConvertBase)
	testconvertbase("256850", "0123456789", "01", "111110101101010010", student.ConvertBase)
	testconvertbase("uuhoumo", "choumi", "Zone01", "eeone0n", student.ConvertBase)
	testconvertbase("683241", "0123456789", "0123456789", "683241", student.ConvertBase)
	fmt.Println()

	color.Cyan("10. RotateVowels test")
	fmt.Println("Arguments: \"Hello World\"")
	fmt.Println("Expected: Hollo Werld")
	fmt.Print("Result: ")
	cmd := exec.Command("/home/student/div-01/div/rotatevowels/rotatevowels", "Hello World")
	cmd.Stdout = os.Stdout
	cmd.Run()
	fmt.Println()
	fmt.Println("Arguments: \"HEllO World\" \"problem solved\"")
	fmt.Println("Expected: Hello Werld problom sOlvEd")
	fmt.Print("Result: ")
	cmd = exec.Command("/home/student/div-01/div/rotatevowels/rotatevowels", "HEllO World", "problem solved")
	cmd.Stdout = os.Stdout
	cmd.Run()
	fmt.Println()
	fmt.Println("Arguments: \"str\" \"shh\" \"psst\"")
	fmt.Println("Expected: str shh psst")
	fmt.Print("Result: ")
	cmd = exec.Command("/home/student/div-01/div/rotatevowels/rotatevowels", "str", "shh", "psst")
	cmd.Stdout = os.Stdout
	cmd.Run()
	fmt.Println()
	fmt.Println("Arguments: \"happy thoughts\" \"good luck\"")
	fmt.Println("Expected: huppy thooghts guod lack")
	fmt.Print("Result: ")
	cmd = exec.Command("/home/student/div-01/div/rotatevowels/rotatevowels", "happy thoughts", "good luck")
	cmd.Stdout = os.Stdout
	cmd.Run()
	fmt.Println()
	fmt.Println("Arguments: \"al's elEphAnt is overly underweight!\"")
	fmt.Println("Expected: il's elephunt es ovirly AndErweaght!")
	fmt.Print("Result: ")
	cmd = exec.Command("/home/student/div-01/div/rotatevowels/rotatevowels", "al's elEphAnt is overly underweight!")
	cmd.Stdout = os.Stdout
	cmd.Run()
	fmt.Println()
	...
	color.Cyan("11. AdvancedSortWordArr test")
	arr1 := []string{"The", "earliest", "computing", "device", "undoubtedly", "consisted", "of", "the", "five", "fingers", "of", "each", "hand"}
	student.AdvancedSortWordArr(arr1, strings.Compare)
	fmt.Println(arr1)
	arr2 := []string{"The", "word", "digital", "comesfrom", "\"digits\"", "or", "fingers"}
	student.AdvancedSortWordArr(arr2, strings.Compare)
	fmt.Println(arr2)
	result := []string{"a", "A", "1", "b", "B", "2", "c", "C", "3"}
	student.AdvancedSortWordArr(result, strings.Compare)
	fmt.Println(result)
	arr3 := []string{"The", "computing", "consisted", "device", "each", "earliest", "fingers", "five", "hand", "of", "of", "the", "undoubtedly"}
	student.AdvancedSortWordArr(arr3, func(a, b string) int { return strings.Compare(b, a) })
	fmt.Println(arr3)
	arr4 := []string{"\"digits\"", "The", "comesfrom", "digital", "fingers", "or", "word"}
	student.AdvancedSortWordArr(arr4, func(a, b string) int { return strings.Compare(b, a) })
	fmt.Println(arr4)
	arr5 := []string{"1", "2", "3", "A", "B", "C", "a", "b", "c"}
	student.AdvancedSortWordArr(arr5, func(a, b string) int { return strings.Compare(b, a) })
	fmt.Println(arr5)
	fmt.Println()
	fmt.Println()

	color.Cyan("12. cat test")
	color.Yellow("run /cat/test.sh")
	fmt.Println()
	fmt.Println()

	color.Cyan("13. ztail test")
	color.Yellow("run /ztail/test.sh")
	fmt.Println()
	fmt.Println()

	color.Cyan("14. ActiveBits test")
	testactivebits(15, 4, student.ActiveBits)
	testactivebits(17, 2, student.ActiveBits)
	testactivebits(11, 3, student.ActiveBits)
	testactivebits(9, 2, student.ActiveBits)
	testactivebits(12, 2, student.ActiveBits)
	testactivebits(2, 1, student.ActiveBits)
	fmt.Println()

	/*color.Cyan("15. SortListInsert test")
	list1 := []int{0}
	expected1 := "0-> 39-> <nil>"
	testsortlistinsert(list1, 39, expected1, student.SortListInsert)
	list1 = []int{0, 1, 2, 3, 4, 5, 24, 25, 54}
	expected1 = "0-> 1-> 2-> 3-> 4-> 5-> 24-> 25-> 33-> 54-> <nil>"
	testsortlistinsert(list1, 33, expected1, student.SortListInsert)
	list1 = []int{0, 2, 18, 33, 37, 37, 39, 52, 53, 57}
	expected1 = "0-> 2-> 18-> 33-> 37-> 37-> 39-> 52-> 53-> 53-> 57-> <nil>"
	testsortlistinsert(list1, 53, expected1, student.SortListInsert)
	list1 = []int{0, 5, 18, 24, 28, 35, 42, 45, 52}
	expected1 = "0-> 5-> 18-> 24-> 28-> 35-> 42-> 45-> 52-> 52-> <nil>"
	testsortlistinsert(list1, 52, expected1, student.SortListInsert)
	list1 = []int{0, 12, 20, 23, 23, 24, 30, 41, 53, 57, 59}
	expected1 = "0-> 12-> 20-> 23-> 23-> 24-> 30-> 38-> 41-> 53-> 57-> 59-> <nil>"
	testsortlistinsert(list1, 38, expected1, student.SortListInsert)
	fmt.Println()
	*/

	color.Cyan("16. SortedListMerge test")
	fmt.Println()

	color.Cyan("17. ListRemoveIf test")
	fmt.Println()

	color.Cyan("18. BTreeTransplant test")
	fmt.Println()

	color.Cyan("19. BTreeApplyByLevel test")
	fmt.Println()

	color.Cyan("20. BTreeDeleteNode test")
	fmt.Println()
}

func testresult(arg string, expected int, f func(string) int) {
	fmt.Printf("Argument: %s\t\n", arg)
	fmt.Printf("Expected: %d\t\n", expected)
	fmt.Println("Result:", f(arg))
	if f(arg) != expected {
		color.Red("FAIL")
	} else {
		color.Green("OK")
...
func testresult2(arg1, arg2, expected int, f func(int, int) int) {
	fmt.Printf("Argument 1: %d\tArgument 2: %d\n", arg1, arg2)
	fmt.Printf("Expected: %d\t\n", expected)
	fmt.Println("Result:", f(arg1, arg2))
	if f(arg1, arg2) != expected {
		fmt.Println("FAIL")
	} else {
		color.Green("OK")
	}
...
func testresultstr(arg1 int, arg2 string, expected string, f func(int, string)) {
	fmt.Printf("Argument 1: %d\tArgument 2: %s\n", arg1, arg2)
...
func testdoop(arg1, arg2, arg3, expected string) {
	fmt.Printf("%s %s %s\n", arg1, arg2, arg3)
	fmt.Printf("Expected: %s\t\n", expected)
	fmt.Print("Result: ")
	cmd := exec.Command("/home/student/div-01/div/doop/doop", arg1, arg2, arg3)
	cmd.Stdout = os.Stdout
	cmd.Run()
	fmt.Println()
}

func testatoibase(arg1, arg2 string, expected int, f func(string, string) int) {
...
func testconvertbase(arg1, arg2, arg3 string, expected string, f func(string, string, string) string) {
	fmt.Printf("nbr: %s\tbaseFrom: %s\tbaseTo: %s\n", arg1, arg2, arg3)
	fmt.Printf("Expected: %s\t\n", expected)
	fmt.Println("Result: ", f(arg1, arg2, arg3))
	if f(arg1, arg2, arg3) != expected {
		fmt.Println("FAIL")
	} else {
		color.Green("OK")
	}
...
func testactivebits(arg int, expected uint, f func(int) uint) {
...
/*
func testsortlistinsert(l []int, dataref int, expected string, f func(*student.NodeI, int) *student.NodeI) {
	fmt.Println("List:")
	var link1 *student.NodeI
	for i := range l {
		link1 = listPushBack(link1, l[i])
	}
	PrintList(link1)
	fmt.Printf("data_ref:\n%d\n", dataref)
	fmt.Printf("Expected:\n%s\n", expected)
	link1 = student.SortListInsert(link1, dataref)
